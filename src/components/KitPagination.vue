...
.kit-pagination {
	...
	/* `safe` — не украшение: при переполнении обычный `center` срезает НАЧАЛО
	   списка, и доскроллить до него нельзя в принципе (оверфлоу на стартовом
	   крае не прокручивается). `safe` в этом случае выравнивает по началу.
	   Двумя строками, потому что браузер без поддержки отбрасывает всё
	   объявление целиком и остался бы вовсе без центрирования. */
	justify-content: safe center;
	gap: var(--kit-spacing-xs);
	margin: 0;
	/* Место обводке фокуса: список — контейнер прокрутки (см. ниже), без
	   отступа outline крайних кнопок срезался бы его краем. */
	padding: 2px;
	list-style: none;
	/*
	 * Клапан. Если слоты всё же не помещаются — четырёхзначные номера,
	 * экран уже всего, что предусмотрено, — прокручивается пейджер, а не
	 * страница. Без него переполнение доставалось документу: на 270px
	 * пейджер начинался с x=-11 и растягивал scrollWidth до 281.
	 */
	overflow-x: auto;
}
...
/*
 * Ниже 320px не помещается и сокращённый набор: пять кнопок по 44px плюс два
 * многоточия — это 292px, а внутри карточки на 270px доступно 254px.
 *
 * Ужимаем пропорции, а не режем слоты дальше: следующими на выход шли бы
 * первая и последняя страницы, а многоточие без них не значит ничего.
 * Тач-цель становится 36×44 — вдвое выше минимума WCAG 2.2 (24×24), тогда
 * как альтернатива (44px и прокрутка) означала бы, что на Galaxy Fold
 * пейджер нужно листать вбок.
 *
 * 320px — не произвольная граница: это ширина, на которой WCAG требует
 * обходиться без горизонтальной прокрутки (reflow при 400% зуме), и там
 * набор помещается как есть.
 */
